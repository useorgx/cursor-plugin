#!/usr/bin/env node
/**
 * OrgX Cursor plugin: emit the execution graph (the WEG keystone) from the
 * Work Graph reconcile path.
 *
 * Cursor's lifecycle hooks SPOOL events to a local outbox (they do not POST
 * live), and there is no session-stop hook event. The plugin's documented way
 * to get that data to OrgX is `orgx-work-graph-reconcile.mjs --post`. So the
  ...
  const nodes = [
    {
      id: "session",
      type: "task",
      title: clampStr(opts.summary, 500) || "Cursor session",
      status: "completed",
...
export function buildExecutionGraphEvent({ records, env, sessionId }) {
  ...
  let sourceClient = pickString(env.ORGX_SOURCE_CLIENT) || "cursor";
  if (!VALID_SOURCE_CLIENTS.has(sourceClient)) sourceClient = "cursor";
  ...
  const event = {
    ...
    edges,
    trust_events: [],
    metadata: { emitter: "orgx-cursor-plugin", via: "work-graph-reconcile" },
  };
