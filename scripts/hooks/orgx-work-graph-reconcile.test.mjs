...
test('work graph reconciler reads Cursor hook outbox jsonl', async () => {
  const outbox = join(mkdtempSync(join(tmpdir(), 'orgx-cursor-reconcile-')), 'events.jsonl');
  ...
  assert.equal(loaded.records[0].source_client, 'cursor');
});
...
test('work graph reconciler dry-run writes report without credentials', async () => {
  const dir = mkdtempSync(join(tmpdir(), 'orgx-cursor-reconcile-main-'));
