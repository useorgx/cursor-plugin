...
const records = [
  { event: "session_start", source_client: "cursor", session_id: "s1", summary: {} },
  { event: "post_tool_use", source_client: "cursor", session_id: "s1", summary: { tool_name: "Edit" } },
  { event: "post_tool_use", source_client: "cursor", session_id: "s1", summary: { tool_name: "Bash" } },
  { event: "post_tool_use_failure", source_client: "cursor", session_id: "s1", summary: { tool_name: "Bash" } },
];
...
test("buildExecutionGraphEvent: cursor source, correlation_id run identity", () => {
  ...
  assert.equal(event.source_client, "cursor");
...
test("maybeEmitExecutionGraph: enabled + configured POSTs the derived graph", async () => {
  ...
  assert.equal(postedBody.source_client, "cursor");
